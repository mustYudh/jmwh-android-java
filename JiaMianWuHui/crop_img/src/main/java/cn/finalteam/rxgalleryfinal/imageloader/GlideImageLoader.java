...
public class GlideImageLoader implements AbsImageLoader {
  ...
  @Override public void displayImage(Context context, String path, FixImageView imageView,
      Drawable defaultDrawable, Bitmap.Config config, boolean resize, boolean isGif, int width,
    ...
    if (isGif) {
      Glide.with(context)
          .load(path)
          .apply(new RequestOptions().placeholder(defaultDrawable)
