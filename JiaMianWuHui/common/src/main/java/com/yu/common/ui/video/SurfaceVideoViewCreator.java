package com.yu.common.ui.video;
...
public abstract class SurfaceVideoViewCreator
    implements SurfaceVideoView.OnPlayStateListener, MediaPlayer.OnErrorListener,
  ...
  private void prepareStart(String videoPath) {
    try {
      ...
      if (videoFile.exists()) {     /** 存在缓存 */
        ...
        String secondCacheFilePath =
            getSecondVideoCachePath(); /** 第二缓存目录，应对此种情况，例如，本地上传是一个目录，那么就可能要到这个目录找一下 */
  ...
  public void onKeyEvent(KeyEvent event) {
    switch (event.getKeyCode()) {// 跟随系统音量走
      ...
      case KeyEvent.KEYCODE_VOLUME_UP:
        if (!getActivity().isFinishing()) surfaceVideoView.dispatchKeyEvent(getActivity(), event);
        break;
    }
  }

  private boolean isWebUrl(String videoPath) {
    return videoPath.contains("https://") || videoPath.contains("http://") || videoPath.contains(
        "ftp://");
  }
  ...
  public boolean onInfo(MediaPlayer mp, int what, int extra) {
    switch (what) {
      ...
      case MediaPlayer.MEDIA_INFO_BUFFERING_END:   /** 缓冲结束 */
        if (!getActivity().isFinishing()) surfaceVideoView.start();
        break;
  ...
  private class MyAsyncTask extends AsyncTask<String, Integer, String> {
    ...
    @Override protected void onPostExecute(String o) {
