...
public class CustomDynamicInterceptor extends BaseDynamicInterceptor<CustomDynamicInterceptor> {
  ...
  private Map<String, String> handleParams(Request request) {
    ...
    if (isLogin) {
      oldParams.put("token", UserProfile.getInstance().getToken());
      oldParams.put("lUserId", UserProfile.getInstance().getAccount());
    }
    ...
    for (Map.Entry<String, String> m : newParams.entrySet()) {
      sing.append(m.getKey()).append("=").append(m.getValue()).append("&");
    }
