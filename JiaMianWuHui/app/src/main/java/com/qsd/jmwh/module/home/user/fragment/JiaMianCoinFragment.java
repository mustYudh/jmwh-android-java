...
public class JiaMianCoinFragment extends BaseFragment
    implements View.OnClickListener, JiaMianCoinViewer {
  @PresenterLifeCycle private JiaMianCoinPresenter mPresenter = new JiaMianCoinPresenter(this);
  ...
  @Override public void getInfo(AccountBalance accountBalance, int type) {
  ...
  @Override public void coinConvertMoney(MaskBallCoinBean maskBallCoinBean) {
    SelectHintPop selectHintPop = new SelectHintPop(getActivity());
    selectHintPop.setMessage(
        "你有" + maskBallCoinBean.nPayFee + "个假面币可以兑换成人民币" + maskBallCoinBean.nMoney + "元，确定申请兑换吗？")
        .setTitle("提现申请")
        .setPositiveButton("确定", v -> {
          mPresenter.withdrawMaskBallCoin(maskBallCoinBean.nPayFee);
          selectHintPop.dismiss();
        })
        .setNegativeButton("取消", v -> selectHintPop.dismiss())
        .showPopupWindow();
  }
