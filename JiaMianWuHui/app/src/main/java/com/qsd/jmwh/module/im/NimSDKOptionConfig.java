...
public class NimSDKOptionConfig {
    ...
    public static SDKOptions getSDKOptions(Context context) {
        ...
        // 通知栏显示用户昵称和头像
        options.userInfoProvider = new NimUserInfoProvider(APP.getInstance());
...
//        options.mNosTokenSceneConfig = createNosTokenScene();
    ...
    public static String getAppCacheDir(Context context) {
        ...
        if (TextUtils.isEmpty(storageRootPath)) {
            ...
            storageRootPath = Environment.getExternalStorageDirectory() + "/" + APP.getInstance().getPackageName();
        }
...
//        config.notificationFolded = false;
    ...
    private static MessageNotifierCustomization
        messageNotifierCustomization = new MessageNotifierCustomization() {
        @Override
