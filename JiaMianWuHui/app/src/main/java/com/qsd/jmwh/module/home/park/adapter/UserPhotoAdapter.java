...
public class UserPhotoAdapter extends BasicAdapter<OtherUserInfoBean.CdoFileListDataBean> {
  ...
  @Override protected BaseHolder<OtherUserInfoBean.CdoFileListDataBean> getHolder(Context context,
      int position) {
    return new BaseHolder<OtherUserInfoBean.CdoFileListDataBean>(context,
        R.layout.item_user_photo_layout) {
      @Override public void bindData(OtherUserInfoBean.CdoFileListDataBean data) {
        ...
        TextView videoTag = findViewId(R.id.video_tag);
        boolean canClick = true;
        if (userId != UserProfile.getInstance().getUserId()) {
          if (isOpenAll) {
            if (data.nAttribute == 0) {
              //普通照片
              if (data.nFileType == 0) {
                //展示照片
