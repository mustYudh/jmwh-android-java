...
public class DateRangeActivity extends BaseBarActivity implements DateRangeViewer {
  ...
  public static Intent getIntent(Context context, int level, String title) {
  ...
  @Override protected void loadData() {
    ...
    mPresenter.getRangeData(intent.getIntExtra(LEVEL, -1), 0, userId, userToken, PROVINCE);
  }
  ...
  private void notifyDataSetChanged(Intent intent, BaseQuickAdapter adapter, int position,
    ...
    if (type == PROVINCE) {
      mPresenter.getRangeData(intent.getIntExtra(LEVEL, -1), range.get(position).lId,
          userId, userToken, CITY);
    }
  }
  ...
  @Override public void setProvince(List<RangeData.Range> provinces) {
    if (provinces.size() > 0) {
      ...
      mPresenter.getRangeData(getIntent().getIntExtra(LEVEL, -1), range.lId, userId, userToken, CITY);
