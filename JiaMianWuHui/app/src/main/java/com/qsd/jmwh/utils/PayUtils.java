 ...
 * @date 2019-04-30
 */
@SuppressLint("CheckResult") public class PayUtils {
  ...
  private PayUtils startPay(Activity context, int type, PayInfo info) {
    if (type == 1) {
  ...
  private void startWXPay(Activity context, int type, PayInfo info) {
    boolean installWeChat = UMShareAPI.get(context).isInstall(context, SHARE_MEDIA.WEIXIN);
    if (installWeChat) {
    ...
    if (needCheck) {
      XHttpProxy.proxy(ApiServices.class)
          ...
          .subscribeWith(new TipRequestSubscriber<PayInfo>() {
            @Override protected void onSuccess(PayInfo payInfo) {
              needCheck = false;
              if (payCallback != null) {
                payCallback.onPaySuccess(type);
              }
