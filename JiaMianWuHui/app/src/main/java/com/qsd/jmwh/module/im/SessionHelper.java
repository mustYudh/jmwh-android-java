...
import com.qsd.jmwh.module.im.viewholder.MsgViewHolderDefCustom;
import com.qsd.jmwh.module.im.viewholder.MsgViewHolderGuess;
import com.qsd.jmwh.module.im.viewholder.MsgViewHolderSnapChat;
import com.qsd.jmwh.module.im.viewholder.MsgViewHolderSticker;
import com.qsd.jmwh.module.im.viewholder.MsgViewHolderTip;
...
public class SessionHelper {
  ...
  public static void startP2PSession(Context context, String account, IMMessage anchor) {
    if (!UserProfile.getInstance().getSimUserId().equals(account)) {
      if (NimUIKit.getRobotInfoProvider().getRobotByAccount(account) != null) {
        NimUIKit.startChatting(context, account, SessionTypeEnum.P2P, getRobotCustomization(),
            anchor);
      } else {
  ...
  private static SessionCustomization getP2pCustomization() {
    if (p2pCustomization == null) {
      p2pCustomization = new SessionCustomization() {
        ...
        @Override public void onActivityResult(final Activity activity, int requestCode,
            int resultCode, Intent data) {
          super.onActivityResult(activity, requestCode, resultCode, data);
        }

        @Override public boolean isAllowSendMessage(IMMessage message) {
          return checkLocalAntiSpam(message);
        }

        @Override public MsgAttachment createStickerAttachment(String category, String item) {
      ...
      SessionCustomization.OptionsButton cloudMsgButton = new SessionCustomization.OptionsButton() {
        ...
        @Override public void onClick(Context context, View view, String sessionId) {
  ...
  private static RecentCustomization getRecentCustomization() {
    if (recentCustomization == null) {
      recentCustomization = new DefaultRecentCustomization() {
        ...
        @Override public String getDefaultDigest(RecentContact recent) {
  ...
  private static void setSessionListener() {
    SessionEventListener listener = new SessionEventListener() {
      ...
      @Override public void onAvatarClicked(Context context, IMMessage message) {
        // 一般用于打开用户资料页面
        ...
        if (TextUtils.isEmpty(account) && account.contains("im_") && !account.equals(
            "im_" + NimUIKit.getAccount()) && !account.equals("Im_1")) {
          int userId = Integer.parseInt(message.getSessionId().replace("im_", ""));
          LauncherHelper.from(context)
              .startActivity(LookUserInfoActivity.getIntent(context, userId, userId, 2));
        }
      }

      @Override public void onAvatarLongClicked(Context context, IMMessage message) {
        // 一般用于群组@功能，或者弹出菜单，做拉黑，加好友等功能
      }

      @Override public void onAckMsgClicked(Context context, IMMessage message) {
  ...
  private static void registerMsgRevokeObserver() {
    NIMClient.getService(MsgServiceObserve.class)
        .observeRevokeMessage(new NimMessageRevokeObserver(), true);
  }
  ...
  private static NIMPopupMenu.MenuItemClickListener listener =
      new NIMPopupMenu.MenuItemClickListener() {

        @Override public void onItemClick(final PopupMenuItem item) {
          switch (item.getTag()) {
                  ...
                  new EasyAlertDialogHelper.OnDialogActionListener() {
                    ...
                    @Override public void doOkAction() {
                      NIMClient.getService(MsgService.class)
                          .clearChattingHistory(item.getSessionId(), item.getSessionTypeEnum());
            ...
            case ACTION_CLEAR_P2P_MESSAGE:
              ...
              alertDialog.addItem("确定", new CustomAlertDialog.onSeparateItemClickListener() {
                ...
                @Override public void onClick() {
                  NIMClient.getService(MsgService.class)
                      .clearServerHistory(item.getSessionId(), item.getSessionTypeEnum());
                  MessageListPanelHelper.getInstance().notifyClearMessages(item.getSessionId());
                }
              });
              ...
              alertDialog.addItem(itemText, new CustomAlertDialog.onSeparateItemClickListener() {
                ...
                @Override public void onClick() {
                  NIMClient.getService(MsgService.class)
                      .clearServerHistory(item.getSessionId(), item.getSessionTypeEnum(), false);
                  MessageListPanelHelper.getInstance().notifyClearMessages(item.getSessionId());
                }
              });
