...
public class LookUserInfoActivity extends BaseActivity
  ...
  @Override public void setUserInfo(OtherUserInfoBean userCenterInfo) {
    ...
    if (UserProfile.getInstance().getSex() == 1) {
      ...
      str = "解锁相册" + userData.dGalaryVal + "假面币";
    }
    bindText(R.id.dGalaryVal, str);
  ...
  @Override public void onClick(View v) {
    switch (v.getId()) {
      ...
      case R.id.dGalaryVal:
        if (UserProfile.getInstance().getSex() == 1) {
          if (isVip) {
            mPresenter.checkOpenImageCount();
          } else {
            SelectHintPop hint = new SelectHintPop(this);
            hint.setTitle("温馨提示").setMessage("解锁相册").setPositiveButton("成为会员免费解锁", v1 -> {
              buyVip();
              hint.dismiss();
            }).setNegativeButton("付费解锁 (" + dGalaryVal + "假面币)", v12 -> {
              mPresenter.buyGalleryPay(userID, dGalaryVal);
              hint.dismiss();
            }).setBottomButton("取消", v13 -> hint.dismiss()).showPopupWindow();
          }
        } else {
          openImage();
        }
  ...
  @Override public void refreshImage() {
    userCenterInfo.bOpenImg = true;
    setUserInfo(userCenterInfo);
    ToastUtils.show("解锁成功");
  }

  @Override public void getOpenImageCount(SubViewCount count) {
    if (count.nSurGalaryViewCount > 0) {
      SelectHintPop selectHintPop = new SelectHintPop(getActivity());
