...
public class RechargeListDialog extends BasePopupWindow {
    ...
    public RechargeListDialog(Context context, List<GoodsInfoBean.CdoListBean> list,PayUtils.PayCallBack payCallBack) {
        ...
        adapter.setOnItemChildClickListener((adapter1, view, position) -> {
            if (view.getId() == R.id.goods_sale_fee) {
                GoodsInfoBean.CdoListBean data = (GoodsInfoBean.CdoListBean) adapter1.getData().get(position);
                    SelectedPayTypePop selePayTypePop = new SelectedPayTypePop(context, data, new PayUtils.PayCallBack() {
                        @Override
